...
for i in range(N_WALLS):
    ...
    canvas.create_rectangle(x1, top, x2, bottom, fill='lightgray', outline='black') # First arguments x1,y1,x2,y2

    canvas.create_text(x1 + 8, (top + bottom)/2, text=f"T{i+1}", fill="black", anchor="w") # First arguments are x,y coordinates

    canvas.create_text((x1 + x2)/2, bottom + 15, text=f"L = {L} m", fill="black")   # First argumetns are x,y coordinates

canvas.create_text(left_margin + N_WALLS * wall_width + 8, (top + bottom)/2, text=f"T{N_WALLS+1}", fill="black", anchor="w")
canvas.create_text(left_margin - 10, (top + bottom)/2, text=f"A = {A} m²", fill="black", angle=90)
...
for i in range(n_arrows):
    ...
    canvas.create_line(arrow_x, y + arrow_length/2, arrow_x, y - arrow_length/2, arrow=tk.LAST, width=2, fill="royalblue") # First inputs are x1,y1, x2, y2 coordintes

canvas.create_text(arrow_x + 15, (top + bottom)/2, text=f"Convection (T∞), h = {h_coef} W⋅m⁻¹K⁻¹", angle=90, fill="royalblue")
...
t1_var = tk.IntVar(value=50)
T2_var = tk.DoubleVar(value=0.0)
T3_var = tk.DoubleVar(value=0.0)
tinf_var = tk.DoubleVar(value=100.0)

t1_label = tk.Label(master=inputs_frame, text="T1 (K):")
t1_scale = tk.Scale(master=inputs_frame, from_=400, to=200, orient=tk.VERTICAL, variable=t1_var)
...
material_menus = [ttk.Combobox(master=materials_frame, values=MATERIAL_OPTIONS, textvariable=wall_vars[i], state='readonly', width=8) for i in range(N_WALLS)]
